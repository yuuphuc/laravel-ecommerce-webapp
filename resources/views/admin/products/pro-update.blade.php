<form id="pro-update" method="POST" class="w-75 p-4 shadow mx-auto">
    @csrf
    <table class="table table-borderless">
        <tr>
            <td><label for="f-proname">Tên sản phẩm:</label></td>
            <td>
                <input type="text" id="f-proname" name="proname" required class="form-control" value="{{ ($product->proname) }}">
            </td>
        </tr>
        <tr>
            <td><label for="f-price">Giá:</label></td>
            <td>
                <input type="number" id="f-price" name="price" required class="form-control" value="{{ ($product->price) }}">
            </td>
        </tr>
        <tr>
            ...
            <td>
                <select name="cateid" id="f-cateid" class="form-select" required>
                    ...
                    <option value="{{ $item->cateid }}" {{ $item->cateid == $product->cateid ? 'selected' : '' }}>
                        {{ $item->catename }}
                    </option>
                    @endforeach
                </select>
        ...
        <tr>
            ...
            <td>
                <textarea name="description" id="f-description" rows="4" class="form-control">{{ ($product->description) }}</textarea>
            </td>
        </tr>
        <tr>
