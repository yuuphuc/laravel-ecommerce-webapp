<form id="orderit-form" method="POST" class="w-50 p-4 shadow mx-auto">
    @csrf
    ...
    <table class="table table-borderless">
        <tr>
            ...
            <td>
                <select id="orderid" name="orderid" class="form-select" required>
                    ...
                    <option value="{{ $order->id }}" {{ old('orderid') == $order->id ? 'selected' : '' }}>
        ...
        <tr>
            ...
            <td><input type="number" id="quantity" name="quantity" class="form-control" value="{{ old('quantity') }}" required></td>
        </tr>
        <tr>
            <td><label for="price">Đơn giá:</label></td>
            <td><input type="number" step="0.01" id="price" name="price" class="form-control" value="{{ old('price') }}" readonly required></td>
        </tr>
        <tr>
            <td colspan="2" class="pt-3">
                <div class="d-flex justify-content-between">
                    <a href="{{ route('orderit.index') }}" class="btn btn-secondary">&lAarr; Quay về</a>
                    <button type="submit" class="btn btn-primary">Thêm chi tiết đơn hàng</button>
                </div>
            </td>
        </tr>
    </table>
